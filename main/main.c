...
static bool is_flash_pin_esp32(int gpio_num) { return (gpio_num >= 6 && gpio_num <= 11); }
static bool is_output_capable_esp32(int gpio_num) { return (gpio_num >= 0 && gpio_num <= 33); }
...
static void validate_gpio_config(void) {
    ...
    const int gpios[] = {
        ...
        CONFIG_ESP_BL0937_CF_GPIO, CONFIG_ESP_BL0937_CF1_GPIO, CONFIG_ESP_BL0937_SEL_GPIO
    };
    for (size_t i = 0; i < sizeof(gpios)/sizeof(gpios[0]); i++) {
        if (is_flash_pin_esp32(gpios[i])) {
            fatal_gpio_config("One or more GPIOs are 6..11 (SPI flash pins) on ESP32-WROOM.");
        }
    }
    if (!is_output_capable_esp32(RELAY_GPIO)) fatal_gpio_config("Relay GPIO must be 0..33 on ESP32.");
    if (!is_output_capable_esp32(BLUE_LED_GPIO)) fatal_gpio_config("Blue LED GPIO must be 0..33 on ESP32.");
    if (!is_output_capable_esp32(RED_LED_GPIO)) fatal_gpio_config("Red LED GPIO must be 0..33 on ESP32.");
    if (!is_output_capable_esp32(CONFIG_ESP_BL0937_SEL_GPIO)) fatal_gpio_config("BL0937 SEL GPIO must be 0..33 on ESP32.");

    if (BUTTON_GPIO >= 34 && BUTTON_GPIO <= 39) {
        ESP_LOGW(CFG_TAG, "Button GPIO %d has NO internal pullups (34..39). Use external pull-up.", BUTTON_GPIO);
    }
#endif
}

static inline void relay_write(bool on)     { gpio_set_level(RELAY_GPIO, on ? 1 : 0); }
 ...
 * This library can crash during /accessories if notified concurrently.
 */
static void relay_set_state(bool on) {
    if (relay_on == on) return;
    relay_on = on;
...
static homekit_value_t outlet_in_use_get(void) { return HOMEKIT_BOOL(relay_on); }

// Energy getters: HomeKit will call these from its own thread while building /accessories or on reads.
// No background notify needed -> no concurrency crash.
...
homekit_accessory_t *accessories[] = {
    HOMEKIT_ACCESSORY(
        .id = 1,
        .category = homekit_accessory_category_outlets,
...
void app_main(void) {
    ...
    // Start BL0937 (non-fatal if it fails; see your patched bl0937_start_default())
